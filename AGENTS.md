...
※2026-09-05から、まとめより後ろに置かれた広告ブロックは **ビルド時に自動でまとめ直前へ移動される**（`lib/posts.ts` の `hoistAffiliateBlocks`）。末尾配置が何度も再発したための保険。ただし原稿の見た目と公開後の位置がずれると混乱するので、**原稿でもまとめ直前に置くこと**。
...
- **出典は Yahoo!ニュース・Yahoo!ファイナンス・dメニューニュースなどの転載ページではなく、配信元（新聞社・通信社・出版社）の記事にリンクする。** 転載ページは一定期間で消える。2026-09-20の点検で、切れていた出典13本のうち11本がYahoo!経由だった
- 見出しで配信元の記事が見つからないときは、各社のサイト内検索（毎日・朝日・産経・時事などはブラウザで開ける）で探す
- 数字の出どころは、報道より発表元（官公庁・日銀・会社のIR）が確実。決算の数字は決算短信にリンクする
- テレビ番組の見逃し配信（TVer・テレ東BIZなど）は期限で消えるので、リンクは付けずに番組名と放送日を書く
...
記事「40歳で十勝に帰って、95歳まで」(lifeplan-one-line-95) の試算は
`scripts/lifeplan.mjs` で再現できる。**年に1回、月次の資産公開が出たあとに回して、
線とのずれを見る**（記事が「毎年答え合わせする」と約束している）。

```bash
node scripts/lifeplan.mjs
```

- 更新するのはスクリプト冒頭の **現在年齢と現在資産の2つだけ**
- 前提（実家月12.5万・iDeCo1,285万・年金実質月7万など）の出どころは
  スクリプトのヘッダーコメントに書いてある。変えるときは元記事との整合を確認する
- 大きくずれていたら、記事に追記するか、続編の候補にする
