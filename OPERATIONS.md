...
- [ ] **「——」（全角ダッシュ）を使っていないか**（AIっぽくなるので禁止。「。」や括弧に置き換える）
- [ ] 「?。」のような句読点の連続や、行頭に浮いた「。」がないか

文体チェックはこのコマンド1発で確認できる（**何も表示されなければ合格**）:
```bash
cd ~/Documents/harukamuy-blog
grep -nE "——|？。|\?。|^[、。]" content/posts/*.md
```
※ 過去に別セッションで書いた記事に「——」が復活していた事例あり（2026-07-04に14箇所修正）。新記事を公開するたびに回すのが安全。
