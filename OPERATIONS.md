...
- **月1回の戦略レビューは `node scripts/strategy-review.js`**（月別PV・流入元・人気ページ・国別・Search Console傾向を一括出力。結果をClaudeの戦略検討セッションに貼れば翌月の方針が決められる）
...
- [ ] 広告ブロック（`:::cta`/`:::product`）は **まとめ見出しの直前** に置いたか（関連記事・免責の後ろはNG。→13章）
...
## 13. 広告ブロック（証券CTA・書籍カード）の置き場所

- `:::cta` / `:::product` は **「## まとめ」見出しの直前**（まとめ見出しが無い記事は、まとめボックス `summary-box` の直前）に置く。
- **関連記事や免責事項の後ろには置かない。** 末尾に置いていた時期は、月3,000PVに対してクリックが月2回だった（読者がそこまで到達しない）。
- 1記事に置く広告ブロックは **合計2個まで**（例: 証券CTA 1 ＋ 書籍 1）。合うものが無ければ入れなくてよい。
- どの広告主・どの本を選ぶかは `AGENTS.md`（アフィリエイト章）と `BOOKS.md` を見る。
